...
def check_assignee(event_id: int, user_id: int, db: Session):
    """Đảm bảo người được giao task (assignee) phải là thành viên hợp lệ và đang hoạt động của sự kiện"""
...
def check_task_permission(task: EventTask, event: Event, user: User):
    """Kiểm tra quyền cập nhật task: Chỉ cho phép OWNER của sự kiện HOẶC chính người được giao task đó"""
...
def list_tasks(event_id, search, task_status, priority, assignee_id, page, size, sort_by, sort_order, db: Session):
    """Truy vấn danh sách Task hỗ trợ đa bộ lọc (search, status, priority, assignee) và Phân trang (Pagination)"""
    ...
    # Động hóa sắp xếp theo hạn chót (due_date) hoặc ngày tạo (created_at)
    ...
    if sort_order.lower() == "asc":
        query = query.order_by(sort_order.asc())
    else:
